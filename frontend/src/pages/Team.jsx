...
function TeamMemberCard({ member, index }) {
  ...
  return (
    <motion.article
      ...
      className={`group flex flex-col items-center rounded-[28px] border bg-white p-6 text-center shadow-[0_8px_32px_rgba(15,23,42,0.04)] hover:shadow-[0_20px_48px_rgba(76,29,149,0.1)] hover:-translate-y-1 transition-all duration-500 overflow-hidden relative ${
        member.isDirector
          ? 'border-violet-200 ring-2 ring-[#4C1D95]/10'
          : 'border-gray-200/60'
      }`}
    >
      {/* Director badge */}
      {member.isDirector && (
        <div className="absolute top-4 right-4">
          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-gradient-to-r from-[#3B0764] to-[#6D28D9] text-[9px] font-black uppercase tracking-widest text-white">
            ⭐ Founder
          </span>
        </div>
      )}
      ...
      {/* Profile Image */}
      <div className={`relative mb-5 h-36 w-36 overflow-hidden rounded-full border-4 shadow-[0_8px_24px_rgba(76,29,149,0.12)] ${
        member.isDirector ? 'border-[#4C1D95]/30' : 'border-white'
      }`}>
        <img
          src={member.image}
          alt={member.name}
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-[#4C1D95]/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      </div>

      {/* Name & Role */}
      <h3 className="text-base font-black text-gray-900 group-hover:text-[#4C1D95] transition-colors duration-200">
        {member.name}
      </h3>
      <span className="mt-2 inline-block rounded-full border border-violet-100 bg-violet-50 px-3 py-1 text-[10px] font-extrabold uppercase tracking-widest text-[#6D28D9]">
        {member.role}
      </span>
      ...
      <div className="mt-5 flex gap-2.5">
        {member.linkedin && (
          <a
            ...
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-gray-200 text-gray-400 transition-all duration-200 hover:border-[#4C1D95] hover:bg-[#4C1D95] hover:text-white hover:scale-105 active:scale-95"
            aria-label="LinkedIn"
          >
